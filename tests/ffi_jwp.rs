...
/// Writes the fixtures the other implementations test against.
///
...
/// # Two files, and why
///
/// The issuer's secret key goes in a SEPARATE file. Only the Go tests need
/// it - they exercise the issuer half - and they read it out of this
/// repository directly. The SDK fixture is vendored into siros-sdk-kotlin
/// and siros-sdk-swift by copying, so anything in it is duplicated into two
/// more public repositories, where an unused secret-key-shaped field is
/// nothing but a secret-scanning false positive waiting to happen. Flagged
/// in review on siros-sdk-swift#114; splitting the files means the SDK copy
/// cannot carry it even if someone re-vendors carelessly.
#[test]
fn dump_sdk_fixture() {
  ...
  let doc = json!({
    "README": "Generated by `make sdk-fixture`. Ground truth for the BBS algebra is \
               test-vectors/emlun_reference.json, not this file; this exists so the SDKs can \
               exercise the container and the bindings. Deliberately carries NO issuer secret key: \
               this file is vendored into the SDK repositories, and only the Go issuer tests need \
               one. See test-vectors/go_issuer_fixture.json.",
    "cases": Value::Object(cases),
  });
  std::fs::write(&path, serde_json::to_string_pretty(&doc).unwrap()).expect("write fixture");
  eprintln!("wrote {path}");

  // The issuer half, for the Go smoketest, which reads it from this
  // repository rather than a vendored copy.
  let issuer_doc = json!({
    "README": "The issuer's test signing key, for go-cabi-smoketest. A fixed constant \
               (Scalar::from(0x5150)), not a credential. Kept out of sdk_jwp_fixture.json because \
               that file is copied into two more public repositories where nothing uses it.",
    "issuer_sk": hex::encode(serialize(&[Ser::Scalar(issuer_key().0)])),
  });
  let issuer_path = std::path::Path::new(&path).with_file_name("go_issuer_fixture.json");
  std::fs::write(&issuer_path, serde_json::to_string_pretty(&issuer_doc).unwrap()).expect("write issuer fixture");
  eprintln!("wrote {}", issuer_path.display());
}
